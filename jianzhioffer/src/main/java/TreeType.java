...
//        Stack<Integer> stackPath = new Stack<>();
    ...
    public TreeNode Convert(TreeNode pRootOfTree) {
        ...
        midIteratorTree(pRootOfTree, nodeList);
    ...
    private void midIteratorTree(TreeNode node, List<TreeNode> nodeList){
        if(node != null){
            midIteratorTree(node.left, nodeList);
            nodeList.add(node);
            midIteratorTree(node.right, nodeList);
        }
    }
    ...
    private void preInter(TreeNode node, StringBuilder sb){

        if(node == null){
            sb.append("#");
        }else {
            sb.append(node.val + "!");
            preInter(node.left, sb);
